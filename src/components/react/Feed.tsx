...
export default function Feed() {
  ...
  useEffect(() => {
    const users = ["SOUL#1337", "CyberPunk42", "NeonHacker", "PixelWarrior", "GhostInTheShell", "MatrixBreaker"]
    ...
    const initialLogs: LogEntry[] = Array.from({ length: 20 }, (_, i) => ({
      ...
      action: "placed pixel",
      position: `(${Math.floor(Math.random() * 64)}, ${Math.floor(Math.random() * 64)})`,
    ...
    <div className="h-full">
      <ScrollArea className="h-full">
        <div className="p-2 space-y-1 font-mono text-xs">
          {logs.map((log) => (
            <div key={log.id} className="flex items-start gap-1 group animate-fadeIn">
              <span className="text-purple-700 shrink-0">[{log.timestamp.toLocaleTimeString()}]</span>
              ...
              <div className="w-3 h-3 rounded-sm shrink-0 mt-0.5" style={{ backgroundColor: log.color }} />
